...
public:
    ...
    // Decisions variables 
    vector< vector< vector< vector< LSExpression > > > > x;
    ...
    void solve(int limit){
        ...
        // Decision variables
        // 0-1 decisions variables: x[w][gr][gf][p]=1 if golfer gf is in group gr on week w at position p
        x.resize(nbWeeks);
        for(int w = 0; w < nbWeeks; w++){
            ...
            for(int gr = 0; gr < nbGroups; gr++){
                ...
                for(int gf = 0; gf < nbGolfers; gf++){
					x[w][gr][gf].resize(groupSize);
					for(int p = 0; p < groupSize; p++){
						x[w][gr][gf][p]=model.boolVar();
					}
                }
            }
        }

        // each week, each golfer is assigned to exactly one group and position
        for(int w = 0; w < nbWeeks; w++){
            for(int gf = 0; gf < nbGolfers; gf++){
                LSExpression nbGroupsAssigned = model.sum();
                for(int gr = 0; gr < nbGroups; gr++){
					for(int p=0; p < groupSize; p++){
						nbGroupsAssigned += x[w][gr][gf][p];
					}
                }
		...
		//each week and group, each position is taken exactly once
        for(int w = 0; w < nbWeeks; w++){
            for(int gr = 0; gr < nbGroups; gr++){
				for(int p = 0; p < groupSize; p++){
					LSExpression nbGolfersInPosition = model.sum();
					for(int gf = 0; gf < nbGolfers; gf++){
						nbGolfersInPosition += x[w][gr][gf][p];
					}
					model.constraint(nbGolfersInPosition == 1);
                }
            }
        }

		//every golfer has exactly 2 times in each position
		for(int gf=0;gf<nbGolfers;gf++){
			for(int p=0;p<groupSize;p++){
				LSExpression nbInPosition = model.sum();
				for(int w = 0; w < nbWeeks; w++){
					for(int gr = 0; gr < nbGroups; gr++){
						nbInPosition += x[w][gr][gf][p];
					}
				}
				model.constraint(nbInPosition == 2);
			}
		}
            ...
            for(int gr = 0; gr < nbGroups; gr++){
                ...
                for(int gf0 = 0; gf0 < nbGolfers; gf0++){
                    ...
                    for(int gf1 = gf0+1; gf1 < nbGolfers; gf1++){
						meetings[w][gr][gf0][gf1]= model.and_(x[w][gr][gf0][0], x[w][gr][gf1][0]);
						for(int p0 = 0; p0 < groupSize; p0++){
							for(int p1 = 0; p1 < groupSize; p1++) {
								if(p0 == 0 && p1 == 0)
									continue;
								meetings[w][gr][gf0][gf1] = model.or_(model.and_(x[w][gr][gf0][p0], x[w][gr][gf1][p1]),meetings[w][gr][gf0][gf1]);
							}
						}
                    }
                }
